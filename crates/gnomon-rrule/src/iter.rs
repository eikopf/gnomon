...
/// Generates an occurrence iterator struct with the shared iteration logic.
///
/// The two variants differ only in how they hold the `RecurrenceRule`:
/// - `OccurrenceIter<'a>` borrows it (`&'a RecurrenceRule`)
/// - `OwnedOccurrenceIter` owns it (`RecurrenceRule`)
macro_rules! impl_occurrence_iter {
    (
        $(#[$meta:meta])*
        $vis:vis struct $name:ident $(<$lt:lifetime>)? {
            rule: $rule_ty:ty,
        }
        new($rule_param:ident : $new_rule_ty:ty);
    ) => {
        $(#[$meta])*
        $vis struct $name $(<$lt>)? {
            rule: $rule_ty,
        ...
        impl $(<$lt>)? $name $(<$lt>)? {
            pub fn new($rule_param: $new_rule_ty, dtstart: DateTime) -> Self {
                Self {
                    rule: $rule_param,
                    dtstart,
                    period_index: 0,
            ...
            fn check_termination(&self, dt: DateTime) -> bool {
                let rule: &RecurrenceRule = self.rule.borrow();
        ...
        impl $(<$lt>)? Iterator for $name $(<$lt>)? {
            ...
            fn next(&mut self) -> Option<DateTime> {
                ...
                loop {
                    ...
                    self.buffer = expand::expand_period(
                        self.rule.borrow(),
                        self.dtstart,
                        self.period_index,
                    );
